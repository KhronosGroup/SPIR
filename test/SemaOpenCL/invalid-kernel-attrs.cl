...
void f_kernel_image2d_t( kernel image2d_t image ) { // expected-error {{'kernel' attribute only applies to functions}}
  ...
  read_only int i; // expected-error {{access qualifier can only be used for pipe and image type}}
  __write_only int j; // expected-error {{access qualifier can only be used for pipe and image type}}
}
